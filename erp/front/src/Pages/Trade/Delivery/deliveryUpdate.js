...
const DeliveryUpdate = (props) => {
  ...
  const deliveryDeleteRef = useRef(false);
  const [deliveryDeleteLoading, setDeliveryDeleteLoading] = useState(false);
  ...
  const deliveryUpdate = async (historyData) => {
    if (data.register_date === undefined) { message.warning('접수일을 입력해주세요.'); return null; }
    if (data.engineer_id === undefined) { message.warning('담당자를 입력해주세요.'); return null; }
    if (data.content === undefined) { message.warning('접수 내용을 입력해주세요.'); return null; }

    if (deliveryUpdateRef.current) return;
    ...
    try {
      ...
      deliveryUpdateRef.current = false;
      setDeliveryUpdateLoading(false);
    }
  };

  const deliveryDelete = async () => {
    if (deliveryDeleteRef.current) return;
    deliveryDeleteRef.current = true;
    setDeliveryDeleteLoading(true);
    ...
    try {
      ...
      deliveryDeleteRef.current = false;
      setDeliveryDeleteLoading(false);
    }
  };
  ...
  const handleCompleteConfirm = () => {
    ...
    .then(() => {
      ...
      message.success('납품이 완료 처리되었습니다.');
      window.location.reload();
    });
  };
  ...
  const DeliveryCompleteButton = () => (
      <Button variant="primary" onClick={() => changeCategory(1)} disabled={changeCategoryLoading}>
        {changeCategoryLoading ? '처리 중...' : '완료'}
      </Button>
  );

  const DeliveryCancelButton = () => (
      <Button variant="primary" onClick={() => changeCategory(3)} disabled={changeCategoryLoading}>
              ...
              <Card.Body>
                ...
                <Row>
                  <Col style={{ textAlign: 'right' }}>
                    ...
                    {data.category_2 !== 1 ? (
                        <DeleteButton tradeId={data.id} delete={() => deliveryDelete()} disabled={deliveryDeleteLoading} />
                    ) : null}
                    {data.category_2 === 0 ? <DeliveryInternalButton /> : null}
                    {data.category_2 !== 1 ? (
                        <UpdateButton update={() => deliveryUpdate()} disabled={deliveryUpdateLoading} />
                    ) : null}
            ...
            <Row>
              <Col>
                <Card>
                  ...
                  다른 담당자가 납품을 처리한 경우 기록을 남길 수 있습니다.
                </span>
                  </Card.Header>
                  <Card.Body>
                    <Row>
            ...
            <Row>
              <Col>
                <DeliveryUpdateGrid
                    ...
                    isSubmitting={deliveryUpdateLoading || deliveryDeleteLoading}
                />
              </Col>
            </Row>
        ) : null}
