...
const AsUpdateGrid = (props) => {
  const gridRef = React.createRef(); // Toast Ui Grid Function 쓰기 위해서
  ...
  const asUpdate = () => {
    ...
    props.asUpdate(historyData);
  };

  // 특정 행의 내역 삭제
  const historyRowDelete = (e) => {
    if (e.columnName === 'action') {
      // History Row Data
      let rowData = gridRef.current.getInstance().getRow(e.rowKey);

      // History Id Null 여부에 따라서 삭제방식 변경
      if (rowData.id !== null) {
      ...
      <Card>
        <Card.Header>
          ...
          <Row style={{ float: 'right' }}>
            ...
            <UpdateButton update={() => asUpdate()} />
            <AllDeleteButton allHistoryDelete={() => allHistoryDelete()} />
            <DeleteButton tradeId={props.tradeId} delete={() => props.asDelete()} />
          </Row>
        </Card.Header>
        <Card.Body>
          <Grid
