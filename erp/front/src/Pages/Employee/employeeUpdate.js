...
import requestEngineerDelete from '../../Axios/Engineer/requestEngineerDelete';
...
const EmployeeUpdate = ({ match }) => {
  ...
  const employeeDelete = () => {
    if (data.is_active) {
      message.error('활성 직원은 삭제할 수 없습니다. 먼저 비활성화 해주세요.');
      return;
    }

    Modal.confirm({
      title: '직원 삭제',
      content: `'${data.name}' 직원을 정말 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`,
      okText: '삭제',
      okType: 'danger',
      cancelText: '취소',
      onOk() {
        requestEngineerDelete(match.params.employee_id).then((res) => {
          if (res) {
