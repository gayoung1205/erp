...
def make_trade(trade_list):
            ...
            for history in history_list:
                if trade.category_1 in [5, 6]:
                    ...
                    if history.tax_category == 1:
                        tax = roundTraditional((history.price * 0.1), 0)
    ...
    return {
        "data": receivalble,  # total_receivable 값
        "data_1": total_count,  # total_price 값
...
# python round()의 경우에 오사오입 반올림. 사사오입 반올림이 필요하기 때문에 사용하는 함수. round()를 활용하여 사사오입 방식으로 사용.
...
class RecordView(APIView):
    ...
    def get(self, req):
        """
            ...
            category : 전자문서 분류. 0 = 업무일지, 1 = 휴가신청
                - 현재는 업무일지만 사용 중
            is_accept : 문서보관함페이지에서 전자문서를 불러올 경우 1의 값 -> is_approved=True | is_reject=True인 문서들만 가져옴
            is_all : 로그인 시 보이는 최근 문서 Modal에서 불러오는 모든전자문서파일 가져오기
        """
        category = (
        ...
        if is_all is None:
            if is_accept:
                record = record.filter(Q(is_approved=True) & Q(is_reject=False))
                if len(record) == 0:
                    return ReturnNoContent()
                ...
                record = record.filter(Q(is_approved=False) | Q(is_reject=True))
    ...
    def put(self, req, record_id):
        ...
        with transaction.atomic():
            ...
            if req.user.is_superuser and record.is_submit:
                ...
                if approve and record.is_reject:
                    record.is_reject = False
    ...
    def post(self, req):
        ...
        if data["type"]:
            ...
            if data["type"] == "customer":
                ...
                try:
                    tra = (
                        Tra.objects.filter(category_1=3)
                        .filter(created_date__range=["2022-01-01", "2022-08-01"])
                        .order_by("created_date")
                    )
                except Exception as e:
                    print(e)
                    return ReturnNoContent()
                tra_data = make_trade(tra)
                tra_value = tra.values()
                for i in range(len(tra_value)):
                    tra_value[i]["category_name1"] = get_category_name1(tra_value[i])
                    tra_value[i]["engineer_name"] = (
                    ...
                    if tra_value[i]["completed_content"] != None:
                        tra_value[i]["completed_content"] += tra_data["content"][i]
                    cus = Cus.objects.get(id=tra_value[i]["customer_id"])
                    tra_value[i]["address"] = ""
                    if cus.address_1:
                        tra_value[i]["address"] += cus.address_1
                    if cus.address_2:
                        tra_value[i]["address"] += cus.address_2
                type_datas = tra_value
                file_name = "sell.xlsx"
                sheet_name = "판매내역"
                col_headers = ["등록일", "구분1", "고객명", "고객주소", "완료내역"]
                col_names = [
                    "created_date",
                    "category_name1",
                    "customer_name",
                ...
                try:
                    tra = (
                        Tra.objects.filter(category_1=4)
                        .filter(created_date__range=["2022-01-01", "2022-08-01"])
                        .order_by("created_date")
                    )
                except Exception as e:
                ...
                try:
                    tra = (
                        Tra.objects.filter(category_1=7)
                        .filter(created_date__range=["2022-01-01", "2022-08-01"])
                        .order_by("created_date")
                    )
                except Exception as e:
                ...
                engineer_list = ["김영기", "류승영", "최규민", "고영훈", "이현수"]
                try:
                    file_name = "as.xlsx"
                    ...
                    for engineer in engineer_list:
                        eng = Eng.objects.get(name=engineer)
                        tra = (
                            Tra.objects.filter(category_1=0)
                            .filter(engineer=eng)
                            .filter(created_date__range=["2022-05-01", "2022-05-31"])
                            .order_by("created_date")
                        )
                        tra_data = make_trade(tra)
                        tra_value = tra.values()
                        for i in range(len(tra_value)):
                            ...
                            if tra_value[i]["completed_content"] != None:
                                tra_value[i]["completed_content"] += tra_data[
                                    "content"
                                ][i]
                            cus = Cus.objects.get(id=tra_value[i]["customer_id"])
                            tra_value[i]["address"] = ""
                ...
                engineer_list = ["Kim", "Ryu", "Choi", "Koh", "Lee"]
                try:
                    ...
                    for engineer in engineer_list:
                        cal = (
                            Cal.objects.filter(calendarId=engineer)
                            .filter(created_date__range=["2022-01-01", "2022-04-22"])
                            .order_by("start")
                        )
                        type_datas = CalendarSerializer(cal, many=True).data
                        ...
                        for type_data in type_datas:
                            type_data["engineer_name"] = engineer_name
                        sheet_name = engineer_name
                        col_headers = [
                            "등록일",
                            "시작일",
                            "종료일",
                ...
                """
                    지원팀 업무일지 일괄 추출
                """
                # user_id_list = ["bandary", "sfdio77", "bfcombfcom"]
                # try:
                #     file_name = "daily_work.xlsx"
                #     response = HttpResponse(
                #         content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                ...
                #             .filter(date__range=["2022-01-01", "2022-04-22"])
                #             .order_by("date")
                #         )
                #         type_datas = RecordSerializer(records, many=True).data
                ...
                #         if user_id == "bandary":
                #             user_name = "김영기"
                #         elif user_id == "sfdio77":
                #             user_name = "류승영"
                #         elif user_id == "bfcombfcom":
                #             user_name = "최규민"
